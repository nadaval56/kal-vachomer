...
const SOURCES = [
  ...
  },
  {
    ...
    caps: { steinsaltz: 2 },
    refs: [{
      /* "אשרי הגבר אשר תיסרנו" פותח את הלימוד ותלוי בקטע הקודם. */
      ref: 'Berakhot.5a',
...
const PROBE_EXTRA = ['Gittin.17a.1', 'Gittin.17a.2', 'Gittin.17a.3'];
    ...
    for (const { ref } of src.refs) {
      ...
      try {
        for (const v of await segmentsFor(ref)) {
          ...
          v.segments.forEach((seg, i) => console.log(`   [${i + 1}] ${norm(seg).slice(0, 1200)}`));
        }
